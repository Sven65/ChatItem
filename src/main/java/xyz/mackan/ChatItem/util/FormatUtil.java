...
public class FormatUtil {
	...
	public static String formatMessage(final Player user, final String input) {
		...
		if (!user.hasPermission("essentials.chat.url")) {
	...
	public static String formatString(final Player user, String message) {
		...
		if (user.hasPermission("essentials.chat.color")) {
			supported.addAll(COLORS);
		}
		if (user.hasPermission("essentials.chat.format")) {
			supported.addAll(FORMATS);
		}
		if (user.hasPermission("essentials.chat.magic")) {
		...
		for (ChatColor chatColor : ChatColor.values()) {
			...
			final String node = "essentials.chat." + colorName.toLowerCase(Locale.ROOT);
			// Only handle individual colors that are explicitly added or removed.
			if (!user.isPermissionSet(node)) {
				continue;
			}
			if (user.hasPermission("essentials.chat."+node)) {
		...
		boolean rgb = user.hasPermission("essentials.chat.rgb");
